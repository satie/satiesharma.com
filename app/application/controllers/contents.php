...
class Contents extends Koken_Controller {
	...
	function index()
	{
						...
						$c->exists())
					{
						...
						else
						{
							if (in_array('imagick', get_loaded_extensions()))
							{
								...
								$image->setResourceLimit(6, 1); // Threads
		...
		if ($slug || (isset($id) && strpos($id, ',') === false))
		{
			...
			if ($options['context'] && !in_array($options['context'], array('stream', 'favorites', 'features')) && strpos($options['context'], 'tag-') === false && strpos($options['context'], 'category-') === false)
			{
			...
			if ($content->exists())
			{
				...
				if ($options['context'] && !in_array($original_context, array('stream', 'favorites', 'features')) && strpos($original_context, 'tag-') === false && strpos($original_context, 'category-') === false)
				{
					$options['in_album'] = $a;
				}
				...
				if ($options['context'])
				{
					...
					if (!in_array($original_context, array('stream', 'favorites', 'features')) && strpos($original_context, 'tag-') === false && strpos($original_context, 'category-') === false)
					{
						...
						$final['context']['album'] = $a->to_array(array('auth' => $options['context'] === $a->internal_id));
					...
					else
					{
						...
						if (strpos($original_context, 'tag-') !== false)
						{
						...
						else if (strpos($original_context, 'category-') !== false)
						{
