			...
			switch($this->method)
			{
				...
				case 'put':
					if ($id)
					{
						...
						if (isset($_POST['published']) && !isset($_POST['tags']) && !empty($t->tags))
						{
							$_POST['tags'] = join(',', $t->tags) . ' ';
						}
		...
		else
		{
			...
			if ($final['page_type'] === 'essay' && $page->published)
			{
				...
				if ($options['neighbors'])
				{
					...
					if (strpos($options['context'], 'tag-') !== false)
					{
					...
					else if (strpos($options['context'], 'category-') !== false)
					{
