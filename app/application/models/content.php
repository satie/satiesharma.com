...
class Content extends Koken {
	...
	function to_array($options = array())
	{
		...
		$this->categories->order_by('title', 'ASC')->get_iterated();
		$data['categories'] = array();
		foreach($this->categories as $category)
		{
			$data['categories'][] = array_merge($category->to_array(array('limit_to' => 'content')), array('__koken__' => 'category_contents'));
		}

		$data['albums'] = $koken_url_info->base . 'api.php?/content/' . $data['id'] . '/albums' . ($options['in_album'] ? '/context:' . $options['in_album']->id : '');
