...
class Album extends Koken {
	...
	function context($params, $auth)
	{
		...
		if (isset($params['context']) && strpos($params['context'], 'tag-') !== false)
		{
		...
		else if (isset($params['context']) && strpos($params['context'], 'category-') !== false)
		{
	...
	{
		$options = array_merge( array('with_topics' => true, 'with_covers' => true, 'auth' => false), $options );

		$koken_url_info = $this->config->item('koken_url_info');

		$exclude = array('deleted', 'total_count', 'video_count', 'featured_order');
		...
		foreach($this->categories->order_by('title', 'ASC')->get_iterated() as $category)
		{
			$data['categories'][] = array_merge($category->to_array(array('limit_to' => 'album')), array('__koken__' => 'category_albums'));
		}
		...
		if ($options['with_topics']) {
			...
			{
				$t_arr = $text->to_array();
				if ($t_arr['published'] || $options['auth'])
				{
		...
		if ($options['with_covers']) {
			...
			{
				$covers->where('uploaded_on <=', $options['before']);
