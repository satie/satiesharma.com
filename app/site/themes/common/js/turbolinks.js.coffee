...
nonHtmlLink = (link) ->
  url = removeHash link
  url.match(/\.[a-z]+(\?.*)?$/g) and not url.match(/\.(html|php)?(\?.*)?$/g)
...
notKokenLink = (link) ->
  not link.getAttribute('data-koken-internal')?

ignoreClick = (event, link) ->
  notKokenLink(link) or crossOriginLink(link) or anchoredLink(link) or nonHtmlLink(link) or noTurbolink(link) or targetLink(link) or nonStandardClick(event)
